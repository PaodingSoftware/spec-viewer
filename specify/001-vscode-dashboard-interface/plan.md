...
T01. [✅已完成] 创建 Senatus 数据解析工具类 `extension/src/utils/senatus-parser.js`
- 扫描 `specify/` 目录获取所有主题
...
T02. [✅已完成] 创建仪表盘提供者 `extension/src/providers/dashboard-provider.js`
- 继承 WebviewBase 基类
...
T03. [✅已完成] 在 DashboardProvider 中实现文件监控机制
...
T04. [✅已完成] 在 ResourceUri 工具类中添加仪表盘资源 URI 方法
...
T05. [✅已完成] 创建仪表盘 HTML 模板 `extension/webview/dashboard/dashboard.html`
...
T06. [✅已完成] 创建仪表盘样式文件 `extension/webview/dashboard/dashboard.css`
...
T07. [✅已完成] 创建仪表盘前端脚本 `extension/webview/dashboard/dashboard.js`
...
T08. [✅已完成] 在 package.json 中注册仪表盘命令
...
T09. [✅已完成] 在 package.json 的 menus 中添加仪表盘按钮
...
T10. [✅已完成] 在 extension.js 中注册仪表盘提供者和命令
...
T11. [✅已完成] 实现点击主题打开 discuss.md 功能
...
- **实现位置**: `dashboard.js:102-112` (已在 T07 中实现)

T12. [✅已完成] 实现仪表盘手动刷新功能
- 前端：添加刷新按钮，点击发送 `{ command: 'refresh' }` 消息
- 后端：接收消息后调用 `updatePanelContent()` 重新解析数据
- 显示加载状态（可选）
- **实现位置**: `dashboard-provider.js:70-75`, `dashboard.js:145-149` (已在 T02、T07 中实现)
...
- 测试缺少某些文件的主题目录
- 验证错误处理是否正确忽略异常文件
- **测试方案**: 见 `implementation/T14.md`

T15. [✅已完成] 测试主题适配和跨平台兼容性
- 切换 VSCode 浅色/深色主题，验证样式正确
- 在 Windows、macOS、Linux 上测试路径处理
- 验证 Font Awesome 图标正确显示
- 验证进度条、卡片样式在不同主题下的可读性
- **测试方案**: 见 `implementation/T15.md`

T16. [✅已完成] 修复仪表盘图标不显示、刷新失效和任务列表限制问题
...
T17. [✅已完成] 修复仪表盘图标缺失问题（补充 Font Awesome 图标定义）
...
T18. [✅已完成] 修复阶段标签图标不显示问题（添加 fas 基础类）
...
T20. [✅已完成] 修复仪表盘按钮位置不生效问题（使用 navigation@N 排序）
...
T22. [✅已完成] 修复讨论记录解析失败的问题（正则表达式格式错误）
...
T23. [✅已完成] 修复 HTML 注释样例被误识别为真实数据的问题
...
T26. [✅已完成] 将 research 从 stage progression 中移除，使其成为独立属性
...
T28. [✅已完成] 调整主题列表中研究标识样式，使其与阶段标签尺寸一致
...
T29. [✅已完成] 修复仪表盘交互问题（主题卡片点击限制、研究标识点击、鼠标样式）
- 问题背景：
...
T31. [✅已完成] 调整侧边栏标题栏按钮顺序，将仪表盘按钮移到最右侧
